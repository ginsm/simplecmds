...
const Validate = {
  ...
  validateRules: function(rules) {
    Object.keys(rules).forEach((rule) => {
      ...
      /*
        Map: Returns if the arg types are valid (true).
        Every: Ensures every type returned valid (true).
      */
      const validArgTypes = args.map((arg, id) => {
        const required = (id <= types.required.length - 1);

        if (required) {
          return types.required[id].includes(argTypes[id]);
        } else {
          const optionalAmount = (types.optional.length - 1);
          const optionalId = (id - types.required.length);

          if (optionalId > optionalAmount) {
            return types.optional[types.optional.length - 1].includes(argTypes[id]);
          } else {
            return types.optional[optionalId].includes(argTypes[id]);
          }
